...
| `enable_changelog` | `CHANGELOG.md` plus a PR-only `changelog.yml` check: a PR that touches `src/` or `pyproject.toml` must also touch `CHANGELOG.md`, or carry the `skip-changelog` label |
